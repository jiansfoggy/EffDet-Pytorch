...
class EfficientdetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #img, y = self.get_random_data(index, self.image_size[0:2], random=self.is_train)
        hyp = self.hyp
        ...
        self.augment = True if random.random()<0.35 else False
        if self.augment:
            # Augment imagespace
            if not mosaic:
...
def random_perspective(img, targets=(), degrees=180, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
