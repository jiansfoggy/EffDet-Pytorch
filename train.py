...
def fit_one_epoch(net,focal_loss,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    with tqdm(total=epoch_size,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        ...
        tb_writer.add_scalar('Train/Confidence Loss', total_c_loss/(1+iteration), epoch+1)
        tb_writer.add_scalar('Train/Regression Loss', total_r_loss/(iteration+1), epoch+1)
        tb_writer.add_scalar("Train/Learning Rate", get_lr(optimizer), epoch+1)    
    net.eval()
    ...
    with tqdm(total=epoch_size_val, desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(genval):
            ...
            with torch.no_grad():
                ...
                val_loss += loss.item()
                v_c_loss += c_loss.item()
                v_r_loss += r_loss.item()

            pbar.set_postfix(**{'total_loss': val_loss / (iteration + 1)})
            pbar.update(1)
        tb_writer.add_scalar('Valid/Confidence Loss', v_c_loss/(iteration+1), epoch+1)
        tb_writer.add_scalar('Valid/Regression Loss', v_r_loss/(iteration+1), epoch+1)
        tb_writer.add_scalar("Valid/Learning Rate", get_lr(optimizer), epoch+1)
     
    tb_writer.add_scalar('Train/Total Loss', total_loss/(epoch_size+1), epoch+1)
    tb_writer.add_scalar('Valid/Valid Loss', val_loss/(epoch_size_val+1), epoch+1)
...
if __name__ == "__main__":
    ...
    #   获得图片路径和标签
    #----------------------------------------------------#
    annotation_path = '/media/data3/jian/Text_Detection/YOLOV3/train.txt'
    valid_path = '/media/data3/jian/Text_Detection/YOLOV3/valid.txt'
    #----------------------------------------------------------------------#
    ...
    #    lines = f.readlines()
    with open(annotation_path, 'r') as fp_train:
            trn_path = fp_train.read().strip().splitlines()    
    with open(valid_path, 'r') as fp_valid:
            vld_path = fp_valid.read().strip().splitlines()

    np.random.seed(10101)
    np.random.shuffle(trn_path)
    np.random.shuffle(vld_path)
    #np.random.shuffle(lines)
    np.random.seed(None)
    num_val = len(vld_path)
    num_train = len(trn_path)
